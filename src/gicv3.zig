...
pub fn gicd_disable_int(irq: irq_no) void {
    regs.reg_gic_gicd_icenabler((irq / regs.gic_gicd_icenabler_per_reg)).* = 1 << (irq % regs.gic_gicd_icenabler_per_reg);
}

//* enable irq
// @param[in] irq irq number
pub fn gicd_enable_int(irq: irq_no) void {
    regs.reg_gic_gicd_isenabler((irq / regs.gic_gicd_isenabler_per_reg)).* = 1 << (irq % regs.gic_gicd_isenabler_per_reg);
}

//* clear a pending interrupt
// @param[in] irq irq number
fn gicd_clear_pending(irq: irq_no) void {
    regs.reg_gic_gicd_icpendr((irq / regs.gic_gicd_icpendr_per_reg)).* = 1 << (irq % regs.gic_gicd_icpendr_per_reg);
}
...
fn gicd_set_target(irq: irq_no, p: u32) void {
    ...
    reg = regs.reg_gic_gicd_itargetsr(irq / regs.gic_gicd_itargetsr_per_reg).*;
...
fn gicd_set_priority(irq: irq_no, prio: u32) void {
    ...
    reg = regs.reg_gic_gicd_ipriorityr(irq / regs.gic_gicd_ipriority_per_reg).*;
    reg &= ~(@as(u32, 0xff) << shift);
    reg |= (prio << shift);
...
fn gicd_config(irq: irq_no, config: u32) void {
    ...
    reg = regs.reg_gic_gicd_icfgr(irq / regs.gic_gicd_icfgr_per_reg).*;
...
pub fn gic_v3_initialize() void {
    ...
    gicd_set_priority(aarch64.timer_irq, 0 << regs.gic_pri_shift); // set priority */
